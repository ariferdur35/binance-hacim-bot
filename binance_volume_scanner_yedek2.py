...
MIN_BAR_CHANGE    = 2.5
COOLDOWN_SECONDS  = 60 * 60  # coin bazlı 1 saat ignore
...
# Global
# =========================
# coin bazlı cooldown tracker
sent_coins = {}  # {symbol: last_signal_ts}
...
async def run_scan(session, subscribers):
    ...
    for i in range(0, len(symbols), CHUNK_SIZE):
        ...
        tasks = [check_volume_spike(session,s) for s in chunk]
...
async def main():
    log.info("="*50)
    log.info("  Binance Hacim Tarayıcı Botu Başlatıldı (Async Profi)")
